    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse res)
        ...
        try (PDDocument doc = new PDDocument()) {
            ...
            try (PDPageContentStream cs = new PDPageContentStream(doc, coverPage)) {
                ...
                // ===== 背景 =====
                drawFilledRoundRect(cs, 0, 0, w, h, 250, 245, 235);

                // ===== 白枠 =====
                float frameM = 40;
                drawFilledRoundRect(cs, frameM, frameM, w - 80, h - 80, 255, 255, 255);
                drawBorderRect(cs, frameM, frameM, w - 80, h - 80, 180, 180, 180, 1.5f);

                // ===== タイトル =====
                drawText(cs, font, 16, w / 2 - 40, h - 120, "旅のしおり", 80, 80, 80);
                drawText(cs, font, 48, w / 2 - 150, h - 180, "YAMAGATA", 40, 40, 40);
                drawText(cs, font, 16, w / 2 - 110, h - 220, "— " + payload.tripDays + "日間の山形旅行 —", 100, 100, 100);

                // ===== 山形地図 =====
                PDImageXObject map = loadImageAny(doc, req, "/images/defaults/yamagata.jpg");
                if (map != null) {
                    cs.drawImage(map, w / 2 - 170, h / 2 - 120, 340, 300);
                }
                ...
                PDImageXObject soba = loadImageAny(doc, req, "/images/cover/soba.png");
