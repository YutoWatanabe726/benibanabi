    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse res)
        ...
        try (PDDocument doc = new PDDocument()) {
            ...
            try (PDPageContentStream cs = new PDPageContentStream(doc, coverPage)) {
                ...
                // ===== 背景（うすいグラデ風）=====
                drawFilledRoundRect(cs, 0, 0, w, h, 250, 245, 240);

                // ===== タイトル（上）=====
                drawText(cs, font, 14, 60, h - 60, "BENIBANABI TRAVEL PLAN", 150, 150, 150);
                ...
                // 影
                drawFilledRoundRect(cs, cardX + 6, cardY - 6, cardW, cardH, 220, 220, 220);
                // 本体
                drawFilledRoundRect(cs, cardX, cardY, cardW, cardH, 255, 255, 255);
                drawBorderRect(cs, cardX, cardY, cardW, cardH, 230, 126, 34, 1.5f);

                // ===== 上部ヒーロー画像 =====
                PDImageXObject hero = null;
                if (!topImages.isEmpty()) {
                    hero = topImages.get(0);
                }

                float heroH = 240;
                if (hero != null) {
                    cs.drawImage(hero, cardX, cardY + cardH - heroH, cardW, heroH);
                } else {
                    drawFilledRoundRect(cs, cardX, cardY + cardH - heroH, cardW, heroH, 240, 240, 240);
                }

                // ===== テキスト情報 =====
                float tx = cardX + 40;
                float ty = cardY + cardH - heroH - 60;

                String title = (payload.courseTitle != null && !payload.courseTitle.isEmpty())
                        ? payload.courseTitle : "旅行プラン";

                // タイトル
                drawText(cs, font, 26, tx, ty, title, 30, 30, 30);
                ty -= 50;

                drawText(cs, font, 18, tx, ty, "■ " + payload.tripDays + " 日間の旅", 60, 60, 60);
                ty -= 28;

                String sp = (payload.startPoint != null) ? payload.startPoint : "";
                String sa = (payload.startAddress != null) ? payload.startAddress : "";
                String st = (payload.startTime != null) ? payload.startTime : "";

                drawText(cs, font, 14, tx, ty, "■ 開始地点： " + sp + " " + sa, 80, 80, 80);
                ty -= 22;

                drawText(cs, font, 14, tx, ty, "■ 開始時間： " + st, 80, 80, 80);
                ty -= 22;
                ...
                drawText(cs, font, 12, tx, cardY + 40, "作成日時： " + nowStr, 140, 140, 140);

                // フッター
                drawText(cs, font, 10, 60, 40, "Generated by Benibanabi Route Maker", 160, 160, 160);
            }
