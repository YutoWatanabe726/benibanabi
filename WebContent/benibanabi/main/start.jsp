...
input[type="text"], input[type="date"], input[type="number"], input[type="time"] { width:100%; padding:8px; margin-top:4px; border-radius:6px; border:1px solid #ccc; }
...
<main>
  <form id="startForm">
    <label for="course-title">コースタイトル</label>
    <input type="text" id="course-title" name="course-title" placeholder="例：山形名所巡りコース" required />

    <label>旅行期間（日数）</label>
    <input type="number" id="trip-days" name="trip-days" min="1" max="30" value="1" required />

    <label>スタート地点</label>
    <div class="radio-group">
      <label><input type="radio" name="start" value="山形駅" checked /> 山形駅</label>
      <label><input type="radio" name="start" value="山形空港" /> 山形空港</label>
    </div>
...
document.getElementById('startForm').addEventListener('submit', (e)=>{
  ...
  const params = new URLSearchParams({
    ...
    startPoint: document.querySelector('input[name="start"]:checked').value,
    address: document.getElementById('address').value,
    startTime: document.getElementById('start-time').value
  });
  window.location.href = `spot_list.jsp?${params.toString()}`;
});
</script>
</body>
</html>
